document.addEventListener('DOMContentLoaded', function () {
  ...
  // --- Element References ---
  const searchBranchContainer = document.getElementById(
    'search-parameters-branch'
  );
  const searchSemesterContainer = document.getElementById(
    'search-parameters-semester'
  );
  const searchSubjectContainer = document.getElementById(
  ...
  function updateSubjects(branch) {
    ...
    const subjects =
      subjectMap[branch] || [].concat(...Object.values(subjectMap));
  ...
  function displayNotes(notes) {
    ...
    const bookmarked = JSON.parse(
      localStorage.getItem('bookmarkedNotes') || '[]'
    );
...
`;
  ...
  // Only keep one event listener block for filters, and ensure it's inside DOMContentLoaded
  ...
  // Easy upload card click handler
  ...
  // Move runQuerySearch inside the main block
