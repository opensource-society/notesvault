...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const loadComponents = async () => {
    ...
    try {
      const promises = []
      if (headerPlaceholder) promises.push(fetch('../components/header.html').then((res) => res.text()))
      if (footerPlaceholder) promises.push(fetch('../components/footer.html').then((res) => res.text()))

      const results = await Promise.all(promises)

      let headerLoaded = false
      if (headerPlaceholder) {
        headerPlaceholder.innerHTML = results.shift()
        headerLoaded = true
      }
      if (footerPlaceholder) footerPlaceholder.innerHTML = results.shift()
